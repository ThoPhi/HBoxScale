...
void setup() {
  ...
  HB_OXY_check();
  LCD_print_init();
  for (byte i = 0; i < WEIGHT_BUFFER_SIZE; i++) {
    weightBuffer[i] = thisNode.weight;
  }
}


long rawHX711Value = 0;
  ...
  switch (fsmMode) {
    ...
    case FSM_MAIN_SCALE_CHECK:
      if (scale_check()) {
        ...
        PRINT_DEBUG(Serial.println(readHX711()));
